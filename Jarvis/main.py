...
TOOL_DECLARATIONS = [
    ...
    },
    {
        ...
        "description": (
            "Shuts down THIS assistant (JARVIS / Mark LIII) only — not other programs. "
            "Call this ONLY when the user wants to quit the assistant itself: "
            "goodbye Jarvis, stop the assistant, close Jarvis, shut yourself down. "
            "If they name ANY other app, game, or window "
            "(e.g. close Chrome, close Arma Reforger, close amor reforger, quit Discord), "
            "call close_application with name=that spoken name. "
            "Never use this tool to close games, browsers, or other windows."
        ),
        "parameters": {
            "type": "OBJECT",
            "properties": {},
...
class JarvisLive:
    ...
    async def _execute_tool(self, fc) -> types.FunctionResponse:
        ...
        rerouted = None
        try:
            from actions.close_application import maybe_reroute_close
            rerouted = maybe_reroute_close(getattr(self, "_last_user_text", "") or "", name, args)
        except Exception:
            rerouted = None
        if rerouted is not None:
            print(f"[JARVIS] 🎯 close reroute ({name}) → {rerouted[:80]}")
            if not self.ui.muted:
                self.ui.set_state("LISTENING")
            return types.FunctionResponse(
                id=fc.id, name=name,
                response={"result": rerouted},
            )
                    ...
                    if response.server_content:
                        ...
                        if sc.input_transcription and sc.input_transcription.text:
                            ...
                            if txt:
                                ...
                                self._last_user_text = " ".join(in_buf).strip()
