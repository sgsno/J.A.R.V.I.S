...
def _hotkey(*keys) -> str:
    _require_pyautogui()
    norm = [str(k).lower().strip() for k in keys if str(k).strip()]
    combo = set(norm)
    if combo == {"alt", "f4"} or combo == {"command", "q"}:
        return (
            "Refused Alt+F4/Quit — that closes whichever window is focused "
            "(often JARVIS). Call close_application with the app name instead."
        )
...
TOOL = {
    ...
    "description": "Direct computer control: type, click, hotkeys, scroll, move mouse, screenshots, find elements on screen. NEVER use hotkey Alt+F4 or Command+Q to close an app — that closes the focused window, which is usually JARVIS. To close a named program (Arma Reforger, Chrome, Discord) you MUST call close_application.",
    "parameters": {
        "type": "OBJECT",
        "properties": {
            "action": {
