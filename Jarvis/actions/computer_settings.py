...
# Never treat these as a user-requested app to close.
_SELF_TITLE_MARKERS = (
    "j.a.r.v.i.s",
    "jarvis",
    "mark liii",
...
_SYSTEM_PROCESS_BLOCKLIST = {
    "explorer.exe", "dwm.exe", "winlogon.exe", "csrss.exe", "services.exe",
    "lsass.exe", "smss.exe", "svchost.exe", "taskhostw.exe", "sihost.exe",
...
def _is_self_window(title: str, pid: int, proc_name: str = "") -> bool:
    if pid and pid == os.getpid():
        return True
    t = (title or "").lower()
    if any(m in t for m in _SELF_TITLE_MARKERS):
        return True
    return False


def _is_system_process(proc_name: str) -> bool:
    return (proc_name or "").lower() in _SYSTEM_PROCESS_BLOCKLIST


def _list_user_windows() -> list[dict]:
    """Visible top-level windows with a title, excluding this assistant."""
    if _OS == "Windows":
...
def _list_windows_win() -> list[dict]:
    ...
    def _cb(hwnd, _lparam):
        ...
        if _is_self_window(title, pid.value, proc) or _is_system_process(proc):
            return True
        found.append({
            "hwnd": int(hwnd),
            "title": title,
...
def _list_windows_mac() -> list[dict]:
    ...
    try:
        ...
        for name in names:
            if _is_self_window(name, 0, name):
                continue
            found.append({"hwnd": None, "title": name, "pid": 0, "proc": name})
    except Exception as e:
        print(f"[Settings] macOS window list failed: {e}")
    return found
    ...
    try:
        ...
        if r.returncode == 0:
            for line in r.stdout.splitlines():
                ...
                if _is_self_window(title, pid, proc):
                    continue
                found.append({"hwnd": bits[0], "title": title, "pid": pid, "proc": proc})
            return found
    except FileNotFoundError:
        pass
    except Exception as e:
        print(f"[Settings] wmctrl list failed: {e}")
    if _PSUTIL:
        for p in psutil.process_iter(["pid", "name"]):
            try:
                name = p.info.get("name") or ""
                pid = int(p.info["pid"])
                if _is_self_window(name, pid, name) or _is_system_process(name):
                    continue
                found.append({"hwnd": None, "title": name, "pid": pid, "proc": name})
            except Exception:
                continue
    return found
...
_SPOKEN_REPLACEMENTS = (
    (re.compile(r"\bamor\b", re.I), "arma"),
    (re.compile(r"\barmor\b", re.I), "arma"),
    (re.compile(r"\barmour\b", re.I), "arma"),
    (re.compile(r"\barmer\b", re.I), "arma"),
)

# Canonical name -> extra tokens that still mean this app (process / engine names).
_KNOWN_APP_ALIASES = {
    "arma reforger": ("arma", "reforger", "enfusion", "armareforger", "arma_reforger"),
}


def _normalize_spoken_name(query: str) -> str:
    q = re.sub(r"\s+", " ", (query or "").strip().lower())
    for pat, repl in _SPOKEN_REPLACEMENTS:
        q = pat.sub(repl, q)
    return q
...
def _query_variants(query: str) -> list[str]:
    import difflib
    q = _normalize_spoken_name(query)
    variants = {q}
    if not q:
        return []
    for canon, extras in _KNOWN_APP_ALIASES.items():
        names = (canon,) + tuple(extras)
        if any(difflib.SequenceMatcher(None, q, n).ratio() >= 0.72 or n in q or q in n
               for n in names):
            variants.add(canon)
...
def _match_score(query: str, title: str, proc: str) -> int:
    import difflib
    variants = _query_variants(query) or [_normalize_spoken_name(query)]
    t = (title or "").lower()
    p = re.sub(r"\.exe$", "", (proc or "").lower())
    p_sp = re.sub(r"[_\-]+", " ", p)
    compact_t = re.sub(r"[^a-z0-9]", "", t)
    compact_p = re.sub(r"[^a-z0-9]", "", p)
    hay_tok = set(re.findall(r"[a-z0-9]+", f"{t} {p} {p_sp}"))
    best = 0
    for q in variants:
        if not q:
            continue
        compact_q = re.sub(r"[^a-z0-9]", "", q)
        if q == t or q == p or q == p_sp:
            return 100
        if compact_q and (compact_q in compact_p or compact_q in compact_t):
            best = max(best, 92)
            continue
        if q in t or q in p or q in p_sp:
            best = max(best, 85)
            continue
        ratio = max(
            difflib.SequenceMatcher(None, q, t).ratio() if t else 0.0,
            difflib.SequenceMatcher(None, q, p_sp).ratio() if p_sp else 0.0,
            difflib.SequenceMatcher(None, compact_q, compact_p).ratio() if compact_q and compact_p else 0.0,
            difflib.SequenceMatcher(None, compact_q, compact_t).ratio() if compact_q and compact_t else 0.0,
        )
        if ratio >= 0.78:
            best = max(best, int(70 + 30 * ratio))
            continue
        q_tok = set(re.findall(r"[a-z0-9]+", q))
        if q_tok and q_tok <= hay_tok:
            best = max(best, 80)
            continue
        overlap = q_tok & hay_tok
        # Ignore tiny words so "the" / "app" cannot pick Chrome.
        overlap -= {"the", "a", "an", "app", "game", "program", "window"}
        if q_tok and overlap:
            best = max(best, 40 + 18 * len(overlap))
    return best


def _list_close_candidates() -> list[dict]:
    """Windows plus running processes, so games are found even with odd titles."""
    windows = _list_user_windows()
    seen_pids = {int(w["pid"]) for w in windows if w.get("pid")}
    if not _PSUTIL:
        return windows
    extra: list[dict] = []
    try:
        for p in psutil.process_iter(["pid", "name"]):
            try:
                ...
                if _is_self_window(name, pid, name) or _is_system_process(name):
                    continue
                extra.append({"hwnd": None, "title": name, "pid": pid, "proc": name})
            except Exception:
                continue
    except Exception:
        pass
...
def _pick_windows(target: str, windows: list[dict]) -> list[dict]:
    if not windows:
        return []
    ...
    scored = []
    for w in windows:
        s = _match_score(target, w.get("title", ""), w.get("proc", ""))
        if s >= 55:
            scored.append((s, w))
    ...
    if not scored:
        return []
    best = scored[0][0]
    return [w for s, w in scored if s >= best - 5]


def _close_hwnd_win(hwnd: int) -> bool:
    import ctypes
    from ctypes import wintypes
    user32 = ctypes.windll.user32
    WM_CLOSE = 0x0010
    ...
    try:
        ...
        print(f"[Settings] WM_CLOSE failed: {e}")
        return False


def _close_pid(pid: int) -> bool:
    ...
    if _PSUTIL:
        try:
            ...
            print(f"[Settings] terminate pid {pid} failed: {e}")
    if _OS == "Windows":
        try:
            subprocess.run(
...
def _close_mac_app(name: str) -> bool:
    try:
        r = subprocess.run(
...
def _close_linux_window(win: dict) -> bool:
    hwnd = win.get("hwnd")
    if hwnd:
        try:
            r = subprocess.run(
    ...
    if title:
        try:
            ...
            pass
    return _close_pid(int(win.get("pid") or 0))


def _format_window_list(windows: list[dict], limit: int = 8) -> str:
    if not windows:
        return "none visible"
    lines = []
    for w in windows[:limit]:
        proc = w.get("proc") or "?"
        lines.append(f"{w.get('title', '?')} ({proc})")
...
def _extract_close_target(text: str) -> str | None:
    """Pull an app name out of 'close Arma Reforger' / 'quit chrome'."""
    low = (text or "").strip()
    if not low:
        return None
    m = re.match(
        r"^(?:hey(?:\s+jarvis)?[,\s]+)?(?:please\s+|can you\s+|could you\s+|would you\s+)*"
        r"(?:close|quit|exit|kill)\s+"
        r"(?:the\s+)?(?:app|application|window|program|game|process)?\s*"
        r"(.+)$",
        low,
        flags=re.IGNORECASE,
    )
    ...
    rest = re.sub(r"^(the|my)\s+", "", rest, flags=re.IGNORECASE).strip()
    fillers = {
        "", "this", "it", "that", "them",
        "window", "the window", "app", "the app", "program", "this window",
        "this app", "this program",
    }
    if rest.lower() in fillers:
        return ""
    return rest


def close_named_target(target: str = "", window_only: bool = False) -> str:
    """Close a specific app/window. Never sends Alt+F4 to this assistant."""
    target = (target or "").strip()
    if target and any(m in target.lower() for m in _SELF_TITLE_MARKERS + ("yourself", "the assistant")):
        return (
            "That would close me. Say goodbye if you want me to shut down. "
            "To close a game or another app, name it — for example 'close Arma Reforger'."
        )
    windows = _list_close_candidates()
    matches = _pick_windows(target, windows)

    if not matches:
        if target:
            listing = _format_window_list(windows)
            return (
                f"I could not find an open window matching '{target}'. "
                f"Visible windows: {listing}. "
                "Name the window title or the process (for example 'Arma Reforger')."
            )
        return (
            "I will not close myself. Name the app to close "
            "(for example 'close Arma Reforger'), or say goodbye to shut me down."
        )

    closed: list[str] = []
    pids: set[int] = set()
    for w in matches:
        label = w.get("title") or w.get("proc") or "window"
        pid = int(w.get("pid") or 0)
        if _is_self_window(w.get("title", ""), pid, w.get("proc", "")):
            continue
        ok = False
        if _OS == "Windows" and w.get("hwnd"):
            ok = _close_hwnd_win(int(w["hwnd"]))
        elif _OS == "Darwin":
            ok = _close_mac_app(w.get("proc") or w.get("title") or target)
        else:
            ok = _close_linux_window(w)
        if ok:
            closed.append(label)
            if pid:
                pids.add(pid)

    if not window_only and pids and _OS == "Windows":
        # Games often ignore WM_CLOSE; stop the process if it is still running.
        time.sleep(0.6)
        still = []
        for pid in pids:
            if _PSUTIL:
    ...
    if not closed:
        return (
            f"I found '{matches[0].get('title')}' but could not close it. "
            "Try closing it from the taskbar, or name the process exactly."
        )
    unique = list(dict.fromkeys(closed))
    if target:
        return f"Closed {', '.join(unique)}."
    return f"Closed the previous window: {unique[0]}."


def close_app(target: str = ""):
    return close_named_target(target, window_only=False)


def close_window(target: str = ""):
    return close_named_target(target, window_only=True)
...
_ALIASES = {
    ...
    "close_window":    ("close this window", "close the window"),
    "close_app":       ("quit the app", "quit that program"),
...
def _detect_action(description: str) -> dict:
    ...
    # 1b. "close Arma Reforger" / "quit chrome" — named program, not this HUD.
    close_target = _extract_close_target(raw)
    if close_target is not None:
        if close_target.lower() in ("jarvis", "yourself", "the assistant", "assistant", "mark"):
            return {"action": "", "value": None}
        return {"action": "close_app", "value": close_target}
...
def computer_settings(
    ...
    if action in ("close_app", "close_window", "quit_app", "quit", "kill_app"):
        target = str(
            params.get("target")
            or params.get("app")
            or params.get("window")
            or (value if value is not None else "")
            or ""
        ).strip()
        if not target and description:
            extracted = _extract_close_target(description)
            if extracted:
                target = extracted
            elif extracted is None:
                low_d = description.strip().lower()
                if low_d not in ("close", "close_app", "close app",
                                 "close_window", "close window", "quit"):
...
TOOL = {
    ...
    "description": "Controls the computer: volume, brightness, window management, keyboard shortcuts, typing text on screen, closing OTHER apps by name, fullscreen, dark mode, WiFi, restart, shutdown, scrolling, tab management, zoom, screenshots, lock screen, refresh/reload page. Use for ANY single computer control command. To close a named program (Chrome, Arma Reforger, Discord) you MUST use action=close_app with target set to that name — do not Alt+F4, do not call shutdown_jarvis. restart, shutdown and toggle_wifi put a confirmation on the user's screen and do NOT happen until they press it — never claim they are done. Volume, brightness and dark mode can be reversed with the `undo` tool.",
    "parameters": {
        "type": "OBJECT",
        "properties": {
            ...
            "action": {
                ...
                "description": (
                    ...
                    "pause_video | close_app (requires target=the app name) | close_window | full_screen | "
            ...
            "value": {
                "type": "STRING",
                "description": "Optional value: volume level 0-100, text to type, key name, or app name for close_app."
            },
            "target": {
                "type": "STRING",
                "description": (
                    "Required for close_app / close_window: the app or window to close "
                    "(e.g. 'Arma Reforger', 'Chrome', 'Discord'). "
                    "Never leave this empty when the user named a program. "
                    "Never set this to Jarvis or the assistant."
                )
            }
        },
        "required": []
